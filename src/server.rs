//! Server implementation for the `tunnel-rs` service.
...
impl Server {
    ...
    async fn create_listener(&self, port: u16) -> Result<TcpListener, &'static str> {
        ...
        if port > 0 {
            ...
            for _ in 0..150 {
                let port = fastrand::u16(*self.port_range.start().min(65535).max(*self.port_range.start()));
                if *self.port_range.start() <= port && port <= *self.port_range.end() {
        ...
        match stream.recv_timeout().await? {
            ...
            Some(ClientMessage::Accept(id)) => {
                ...
                match self.conns.remove(&id) {
                    Some((_, mut stream2)) => {
                        // Close our copy - the client's copy will be handled client-side
                        let _ = stream2;
                    }
